...
class Node:
    ...
    def bloquear(self, nodo):
        existe = existe_vecino(self.bloqueados, nodo)
        if existe == False:
            self.bloqueados.append(nodo)
...
class Graph:
    ...
    def hacer_privada(self, nombre):
        existe = existe_usuario(self.usuarios, nombre)
        if existe == True:
            usuario = obtener_usuario(self.usuarios, nombre)
            usuario.privada = True

    def bloquear(self, usuario_1, usuario_2):
        existe_1 = existe_usuario(self.usuarios, usuario_1)
        existe_2 = existe_usuario(self.usuarios, usuario_2)
        if existe_1 == True and existe_2 == True:
            nodo_1 = obtener_usuario(self.usuarios, usuario_1)
            nodo_2 = obtener_usuario(self.usuarios, usuario_2)
            nodo_1.bloquear(nodo_2)
    ...
    def conectar(self, u, v):
        ...
        if existe_u == True and existe_v == True:
            ...
            bloqueado_1 = existe_vecino(nodo_u.bloqueados, nodo_v)
            bloqueado_2 = existe_vecino(nodo_v.bloqueados, nodo_u)
            if bloqueado_1 == False and bloqueado_2 == False:
        ...
        while len(cola) > 0:
            ...
            while i < len(actual.vecinos):
                ...
                if visitado == False:
                    ...
                    bloqueado_1 = existe_vecino(nodo_inicio.bloqueados, vecino)
                    bloqueado_2 = existe_vecino(vecino.bloqueados, nodo_inicio)
                    if vecino.privada == False:
...
def main():
    ...
    while linea != "":
        ...
        elif comando == "privada":
            usuario = partes[1]
            grafo.hacer_privada(usuario)
        elif comando == "bloquear":
            usuario_1 = partes[1]
            usuario_2 = partes[2]
            grafo.bloquear(usuario_1, usuario_2)
        elif comando == "contar":
            usuario = partes[1]
